...
def test_active_listing_requires_authentication(tmp_path):
    async def scenario():
        path = str(tmp_path / "loans.db")
        seed_database(path)
        ...
        try:
            with pytest.raises(ORBError) as error:
                await Stub("EmprestimoService", "127.0.0.1", port).invoke_async("listarEmprestimosAtivos", "usuario-001")
